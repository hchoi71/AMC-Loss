...
# Load trained model
if flgs.AMC is True:
    load_path = './model/cifar10/AMC/'
    save_fig = './result/cifar10/' + 'AMC' + '/'
else:
    load_path = './model/cifar10/Baseline/' 
    save_fig = './result/cifar10/' + 'Baseline' + '/'
...
if __name__ == "__main__":
    ...
    if flgs.AMC is True:
        ...
        merged_tar = tf.where(tf.equal(mask_ph, 0), target_hard, y_ph)
        ...
        geo_desic = tf.acos(tf.clip_by_value(inner, -1.0+1e-07, 1.0-1e-07))
    ...
    with tf.Session() as sess:
        ...
        new_saver = tf.train.import_meta_graph(load_path + 'model_final.meta')
        ...
        for tt in range(x_test.shape[0] // tt_bs):
            ...
            test_fetches = [test_acc, tf.nn.l2_normalize(eval_h_embed, axis=1)]
            tst_v = sess.run(test_fetches, feed_dict={x_ph: test_x_batch, y_ph: test_y_batch, is_training: False})
            
            # Evaluate clustering metrics
            prob = sess.run(target_pred, feed_dict={x_ph: test_x_batch, y_ph: test_y_batch, is_training: False})
            pred_label = np.argmax(prob, axis=1)
            col_pred_label.append(pred_label)
            ###
            t_acc = tst_v[0]
        ...
        pred_labels = np.concatenate(col_pred_label)
        print('>> Test accuracy: {:.2f}%'.format(tt_acc))        
        print('>> V Measure_Score : {}, \n   Homogeneity_Score : {}, \n   Completeness_Score : {}'.format(v_measure_score(y_test, pred_labels,beta=1.0),
              homogeneity_score(y_test, pred_labels), 
              completeness_score(y_test, pred_labels)))
                
        ## Visualize TNSE plot
        test_h = np.vstack(tt_embs)[:10000]
